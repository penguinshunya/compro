...
void solve() {
  ...
  sort(all(ys));
  auto yindex = [&](P y) {
    int x = lower_bound(all(ys), y) - ys.begin();
    return x;
  };
  ...
  rep(i, N) {
    auto [x, y, z] = rows[i];
    int yi = yindex(P(y, i));
    seg.set(yi, make_pair(z, i));
  }
  int l = 0, r = 0;
  while (r < N) {
    while (r < N) {
      auto [ax, ay, az] = rows[l];
      auto [bx, by, bz] = rows[r];
      if (ax == bx) r++;
      else break;
    }
    for (int i = l; i < r; i++) {
    ...
    for (int i = l; i < r; i++) {
      auto [x, y, z] = rows[i];
      int yi = yindex(P(y + 1, -1));
      auto [mz, _] = seg.prod(yi, N);
      if (mz > z) {
        cout << "Yes" << endl;
        return;
      }

...
void solve() {
  ...
  rep(i, N) seg.set(i, P(A[i], i));
  int l = 0, r = pq.top().first + 1;
  vector<int> ans;
  while (ans.size() < M) {
    auto [x, i] = seg.prod(l, r);
    ans.push_back(x);
    l = max(l, i + 1);
    for (auto idx : vecs[x]) {
      seg.set(idx, P(INF, -1));
    ...
    used[x] = true;
    while (pq.size() > 0 && used[pq.top().second])
      pq.pop(), r = pq.top().first + 1;
  }
  cout << ans << endl;
}

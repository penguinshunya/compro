...
void solve() {
  i64 L, N1, N2;
  cin >> L >> N1 >> N2;
  vector<P> acc1(N1), acc2(N2);
  rep(i, N1) {
    i64 v, l;
    cin >> v >> l;
    if (i > 0) l += acc1[i - 1].first;
    acc1[i] = P(l, v);
  }
  rep(i, N2) {
    i64 v, l;
    cin >> v >> l;
    if (i > 0) l += acc2[i - 1].first;
    acc2[i] = P(l, v);
  }
  i64 ans = 0;
  i64 l = 0, r = 0, pos = 0;
  while (l < N1 || r < N2) {
    i64 right = LINF;
    if (l < N1) chmin(right, acc1[l].first);
    if (r < N2) chmin(right, acc2[r].first);
    if (acc1[l].second == acc2[r].second) {
      ans += right - pos;
    }
    if (l < N1 && acc1[l].first == right) l++;
    if (r < N2 && acc2[r].first == right) r++;
    pos = right;
  }
  cout << ans << endl;
}

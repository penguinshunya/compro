...
void solve() {
  ...
  auto rcomp = Compress<int>(R);
  segtree<S, op, e> seg(rcomp.size());
  rep(i, N) {
    int idx = rcomp.get(R[i]);
    seg.set(idx, seg.get(idx) + X[i]);
  }
  vector<int> ord(N);
  iota(all(ord), 0);
  ...
  int l = 0, r = 0;
  while (l < N) {
    while (r < N && C[ord[l]] == C[ord[r]]) {
      r += 1;
    }
    i64 sum = 0;
    repi(i, l, r) {
      sum += X[ord[i]];
      int idx = rcomp.get(R[ord[i]]);
      seg.set(idx, seg.get(idx) - X[ord[i]]);
    }
    chmax(ans, seg.all_prod() + sum);
    repi(i, l, r) {
      int idx = rcomp.get(R[ord[i]]);
      seg.set(idx, seg.get(idx) + X[ord[i]]);
    }

...
void solve() {
  int N, A, B, P, Q;
  cin >> N >> A >> B >> P >> Q;
  auto calc = [&](int s, int ma) {
    auto dp = vector(N + 1, vector(N + 1, mint(0)));
    dp[0][s] = 1;
    rep(i, N) rep(j, N) {
      reps(k, ma) dp[i + 1][min(j + k, N)] += dp[i][j] / ma;
    }
    auto ret = vector(N + 1, mint(0));
    repc(i, N) ret[i] = dp[i][N];
    return ret;
  };
  auto adp = calc(A, P);
  auto bdp = calc(B, Q);
  mint ans = 0;
  rrep(i, bdp.size() - 1) bdp[i] += bdp[i + 1];
  rep(i, adp.size()) ans += adp[i] * bdp[i];
  cout << ans.val() << endl;
}

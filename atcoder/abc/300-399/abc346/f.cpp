...
void solve() {
  ...
  i64 M = S.size();
  i64 ok = 0, ng = LINF;
  auto sums = vector(26, vector(M + 1, 0ll));
  rep(i, M) {
    sums[S[i] - 'a'][i + 1]++;
    rep(j, 26) sums[j][i + 1] += sums[j][i];
  }
  while (ng - ok != 1) {
    i64 md = (ok + ng) / 2;
    i64 curr = 0;
    ...
    rep(i, T.size()) {
      ...
      int ci = curr % M;
      int x = lower_bound(sums[cu].begin() + ci + 1, sums[cu].end(),
                          sums[cu][ci] + rest) -
              sums[cu].begin();
      if (x <= M) {
        curr = curr - ci + x;
      ...
      curr = curr - ci + M;
      rest -= sums[cu][M] - sums[cu][ci];
      i64 rest_n = N - curr / M;
      if (rest_n <= (rest - 1) / sums[cu][M]) {
        ac = false;
        break;
      }
      i64 ne = (rest - 1) / sums[cu][M];
      curr += ne * M;
      rest -= ne * sums[cu][M];
      assert(rest > 0);
      int y = lower_bound(all(sums[cu]), rest) - sums[cu].begin();
      curr += y;
    }
    if (ac) ok = md;
    else ng = md;

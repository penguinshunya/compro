...
void solve() {
  int N, K;
  cin >> N >> K;
  repc(j, K) rep(k, 2) rep(l, 2) ldp[j][k][l] = -LINF;
  repc(j, K) rep(k, 2) rep(l, 2) rdp[j][k][l] = -LINF;
  ldp[0][0][0] = 0;
  ldp[0][0][1] = 0;
  auto update = [&](int nj, int c, i64 v) {
    if (rdp[nj][0][0] == c) {
      if (rdp[nj][0][1] < v) {
        rdp[nj][0][1] = v;
      }
    } else if (rdp[nj][1][0] == c) {
      if (rdp[nj][1][1] < v) {
        rdp[nj][1][1] = v;
      }
    } else {
    ...
    if (rdp[nj][0][1] < rdp[nj][1][1]) {
      swap(rdp[nj][0][0], rdp[nj][1][0]);
      swap(rdp[nj][0][1], rdp[nj][1][1]);
    }
  };
  rep(i, N) {
    repc(j, K) rep(k, 2) rep(l, 2) rdp[j][k][l] = -LINF;
    ...
    rep(j, K) rep(k, 2) {
      int nj = j + 1;
      rdp[nj][k][0] = ldp[j][k][0];
      rdp[nj][k][1] = ldp[j][k][1];
    }
    // use
    repc(j, K) rep(k, 2) {
      if (ldp[j][k][1] == -LINF) continue;
      if (ldp[j][k][0] == c) continue;
      update(j, c, ldp[j][k][1] + v);
    }
    swap(ldp, rdp);
  }
  i64 ans = ldp[K][0][1];

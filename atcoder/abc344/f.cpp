...
using PA = pair<i64, i64>;

void solve() {
  int N;
  cin >> N;
  auto P = vector(N, vector(N, 0));
  cin >> P;
  auto to = vector(N * N, vector<PA>());
  auto f = [&](int i, int j) { return i * N + j; };
  auto g = [&](int x) { return PA(x / N, x % N); };
  rep(i, N) rep(j, N - 1) {
    int w;
    cin >> w;
    to[f(i, j)].emplace_back(f(i, j + 1), w);
  }
  rep(i, N - 1) rep(j, N) {
    int w;
    cin >> w;
    to[f(i, j)].emplace_back(f(i + 1, j), w);
  }
  auto dp = vector(N, vector(N, unordered_map<int, PA>()));
  dp[0][0][0] = PA(0, ~0);
  rep(i, N) rep(j, N) {
    for (auto [maxp, pa] : dp[i][j]) {
      auto [move, money] = pa;
      auto np = max(maxp, P[i][j]);
      money = ~money;
      for (auto [v, w] : to[f(i, j)]) {
        auto [ni, nj] = g(v);
        auto x = max(0ll, (w - money + np - 1) / np);
        auto nmove = move + x + 1;
        auto nmoney = ~(money + x * np - w);
        if (dp[ni][nj].count(np) == 0 || dp[ni][nj][np] > PA(nmove, nmoney)) {
          dp[ni][nj][np] = PA(nmove, nmoney);
        }
      }
    }
  }
  auto ans = LINF;
  for (auto [k, v] : dp[N - 1][N - 1]) {
    chmin(ans, v.first);
  }
  cout << ans << endl;
}

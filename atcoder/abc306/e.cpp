...
void solve() {
  ...
  set<P> add;
  set<P, greater<>> del;
  rep(i, K) add.emplace(0, i);
  rep(i, N - K) del.emplace(0, K + i);
  i64 sum = 0;
  rep(qi, Q) {
    int x, y;
    cin >> x >> y;
    x--;
    int prev = A[x];
    if (add.find(P(prev, x)) != add.end()) {
      add.erase(P(prev, x));
      sum -= prev;
      if (del.size() == 0 || del.begin()->first <= y) {
        add.emplace(y, x);
        sum += y;
      } else {
        auto p = *del.begin();
        add.insert(p), del.erase(p);
        sum += p.first;
        del.emplace(y, x);
      }
    } else {
      del.erase(P(prev, x));
      if (add.begin()->first > y) {
        del.emplace(y, x);
      } else {
        auto p = *add.begin();
        del.insert(p), add.erase(p);
        sum -= p.first;
        add.emplace(y, x);
        sum += y;
      }
